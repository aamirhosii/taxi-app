...
class Monitor:
    ...
    def _average_wait_time(self) -> float:
        """Return the average wait time of passengers that
         have either been picked
    ...
    def _average_trip_distance(self) -> float:
        ...
        count = 0
        distance = 0
        for act in self._activities[DRIVER].values():
            count = len(self._activities[DRIVER])
            for i in range(len(act) - 1):
                if act[i].description == "pickup" and\
                        act[i + 1].description == "dropoff":
                    distance += manhattan_distance(act[i].location,
                                                   act[i + 1].location)
